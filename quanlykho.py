...
class MainWindow(QMainWindow):
    ...
    def start_capture_map(self):
        #me.takeoff()
        self.thread[3] = control_drone(index=3)
        self.thread[3].start()
...
class Form_thongsoke(QWidget):
    ...
    def close_form(self):
        ...
        self.close()
class control_drone(QThread):
    def __init__(self, index):
        self.index = index
        print("start threading", self.index)
        super(control_drone, self).__init__()

    def run(self):
        global move_left, move_right, move_forward, move_back, rotate_clockwise, rotate_counter_clockwise,takeoff,thoat
        global d_lr, vt_cot
        while True:
            if move_right == 1:
                print('right')
                #me.move_right(int(d_lr))
                move_right = 0
            elif move_left == 1:
                print('left')
                #me.move_left(int(-d_lr))
                move_left = 0
            elif move_forward == 1:
                print('forward')
                #me.move_forward(int(vt_cot))
                move_forward = 0
            elif move_back == 1:
                print('back')
                #me.move_back(int(vt_cot))
                move_back = 0
            elif rotate_clockwise == 1:
                print('clockwise')
                #me.rotate_clockwise(90)
                rotate_clockwise = 0
            elif rotate_counter_clockwise == 1:
    ...
    def detect_orange_object(self,frame):
        ...
        for ct in contours:
            ...
            if area > 2000:
                ...
                cv2.putText(frame, 'orange', (int(x), int(y)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
        return frame
class capture_map(QThread):
    ...
    def flytoPoint(self):
        global vitri,go,thoat,points
        global move_left, move_right, move_forward, move_back,rotate_clockwise,rotate_counter_clockwise
        global yaw, x, y, a, lr, fb, i,cot,ke,hang
        global d_lr,vt_cot
        global co        # cờ báo đã tới vị trí cần đến và chụp hình
        #print(int(tach[0]))

        vt_cot = cot*50 #sua lai *100
    ...
    def drawMap(self,img, start_point):
        ...
        for i in range(n):
            ...
            for h in range(int(cd/cd_cot)+1):
                cv2.line(img, (start_x-20,start_y_2), (start_x+20,start_y_2), (255, 255, 255), thickness=10)
    ...
    def run(self):
        ...
        while True:
            ...
            if thoat == 1:
                # me.land()
                break
            if points[-2][0] == 0 and points[-2][1] == 0:
                img = self.drawPoint(img, points, (150,740), hang)
